...
const ARA_BLOOM_DAMAGE_MULTIPLIER_BY_MARK = [1.0, 1.4, 3.0, 6.5];
...
,
    ...
    BLOSSOM_CURRENT: {
        ...
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            ...
            const heal = markCount > 0
                ? Math.max(1, Math.round(Number(attacker.maxHp ?? 0) * 0.04 * markCount))
                : 0;
    ...
    ARA_BLOOM: {
        ...
        element: '물',
        basePower: 42,
        description: '바다 깊은 곳의 꽃물살을 만개시킵니다. 상대의 물결표식이 1/2/3개일 때 피해가 각각 1.4배/3배/6.5배로 증가하며, 명중하면 표식은 모두 사라집니다.',
