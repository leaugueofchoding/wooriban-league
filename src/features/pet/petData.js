import { applyTeamHealToParticipant } from '../battle/battleTeamTargetEngine';
...
// M25B_SKILLDEX_DESCRIPTION_SYNC_V4
const ARA_BLOOM_DAMAGE_MULTIPLIER_BY_MARK = [1.0, 1.4, 2.5, 4.5];
...
return `'${attacker.name}'이(가) 체력을 ${healAmount} 회복했습니다! ✨`;
    ...
    STATIC_SHOCK: {
        ...
        type: 'common',
        element: null,
        description: '약한 전기 충격을 주며, 22% 확률로 적을 1턴간 경직 상태로 만듭니다.',
        basePower: 15,
    ...
    STELLAR_BLAST: {
        ...
        element: '불',
        basePower: 60,
        description: '초고열의 항성 에너지를 폭발시킵니다. 22% 확률로 상대를 매 턴 최대 체력의 8%씩 화상 도트 피해 상태로 만듭니다.',
    ...
    QUICK_DISTURBANCE: {
        ...
        element: '바람',
        basePower: 20,
        description: '눈에 보이지 않는 빠른 속도로 맴돌아 상대를 45% 확률로 1턴 기절시킵니다.',
        ...
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            ...
            if (Math.random() < 0.45) {
                ...
                log += ` 💫 상대가 기절했다! 문제는 보이지만 이번 턴에는 움직일 수 없다!`;
            }
    ...
    LEECH_SEED: {
        ...
        element: '풀',
        basePower: 30,
        description: '상대의 몸에 씨앗을 뿌려 준 피해의 50%만큼 자신의 체력을 회복합니다.',
    ...
    VINE_WHIP: {
        ...
        element: '풀',
        basePower: 35,
        description: '질기고 억센 덩굴을 휘둘러 상대에게 강력한 찰과상을 입힙니다. 1턴간 상대를 욱신욱신 상태로 만들어 공격력과 방어력을 30% 낮춥니다.',
    ...
    SHOCK_SCRATCH: {
        ...
        element: '번개',
        basePower: 25,
        description: '번개를 두른 손톱으로 할큅니다. 35% 확률로 상대를 1턴 경직시킵니다.',
    ...
    THUNDERSTORM: {
        ...
        element: '번개',
        basePower: 65,
        description: '천둥구름을 불러내 거대한 번개를 내리칩니다. 출전 펫과 대기 펫 각각 25% 확률로 1턴 기절시킵니다.',
    ...
    UPHWA: {
        ...
        element: '불',
        basePower: 65,
        description: '체력 20% 이하 시 위력 1.6배 + 45% 확률로 역전 치명타가 발생합니다. 화상 상태 적에게 추가 1.2배 피해를 주고 화상을 소거합니다.',
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            const attacker = attackerPlayer.pet;
            const defender = defenderPlayer.pet;
    ...
    COUNTER_STANCE: {
        ...
        element: '물',
        basePower: 30,
        description: '부들을 휘둘러 피해를 주고, 다음 턴 상대방 공격의 25%를 되돌려주는 반격 자세를 취합니다.',
    ...
    REED_BOW: {
        ...
        element: '풀',
        basePower: 15,
        description: '부들 화살을 쏴 약간의 풀 타입 피해를 주고, 상대를 1턴간 속박하여 방어 행동, 교체, 도망치기를 봉쇄합니다.',
...
,
    ...
    BLOSSOM_CURRENT: {
        ...
        element: '물',
        basePower: 24,
        description: '벚꽃잎이 섞인 해류로 상대를 감싸 피해를 주고 물결표식을 1개 남깁니다. 표식 수 1개당 자신의 최대 HP 7%를 회복하며, 대기 펫도 회복량의 50%를 함께 회복합니다.',
        ...
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            ...
            const heal = markCount > 0
                ? Math.max(1, Math.round(Number(attacker.maxHp ?? 0) * 0.07 * markCount))
                : 0;

            let activeHealAmount = 0;
            let benchHealTotal = 0;
            let benchHealNames = [];

            if (heal > 0) {
                // M19_BLOSSOM_CURRENT_TEAM_HEAL
                // 출전 펫은 기존 회복량을 유지하고, 대기 펫은 40%만 회복합니다.
                // 쓰러진 펫은 회복하지 않고, 최대 HP를 넘기지 않습니다.
                const teamHealResult = applyTeamHealToParticipant(attackerPlayer, heal, {
                    activeMultiplier: 1,
                    benchMultiplier: 0.5,
                    includeActive: true,
                    includeBench: true,
                ...
                const healedEntries = Array.isArray(teamHealResult?.entries)
                    ? teamHealResult.entries
                    : [];
                const activeEntry = healedEntries.find(entry => entry.isActive);
                const benchEntries = healedEntries.filter(entry => entry.isBench);

                activeHealAmount = Number(activeEntry?.amount ?? 0);
                benchHealTotal = benchEntries.reduce((sum, entry) => sum + Number(entry.amount ?? 0), 0);
                benchHealNames = benchEntries.map(entry => entry.petName || '대기 펫');

                if (teamHealResult?.participant) {
                    const nextParticipant = teamHealResult.participant;
                    const activeIndex = Number(nextParticipant.activePetIndex ?? 0);
                    const healedIndexes = new Set(healedEntries.map(entry => entry.index));

                    const nextTeam = Array.isArray(nextParticipant.team)
                        ? nextParticipant.team.map((pet, index) => {
                            if (!pet) return pet;

                            // 풀피라 실제 회복량이 0이어도 출전 펫은 사용 피드백을 보여줍니다.
                            if (index === activeIndex || healedIndexes.has(index)) {
                                const nextPet = {
                                    ...pet,
                                    status: { ...(pet.status || {}) },
                                };
                                markHealPulse(nextPet, 'blossom');
                                return nextPet;
                            }
                    ...
                    attackerPlayer.activePetId = nextParticipant.activePetId || nextTeam[activeIndex]?.id || attackerPlayer.activePetId || null;
                    attackerPlayer.pet = nextTeam[activeIndex] || nextParticipant.pet || attackerPlayer.pet;

                    // petData 내부의 attacker 지역 변수도 이후 로그와 어긋나지 않게 맞춥니다.
            ...
            const shownActiveHeal = activeHealAmount > 0 ? activeHealAmount : heal;
            log += ` 표식의 물살을 타고 ${shownActiveHeal}의 체력을 회복했습니다!`;

            if (benchHealTotal > 0) {
                const shownBenchNames = benchHealNames.slice(0, 2).join(', ');
                const moreBenchCount = Math.max(0, benchHealNames.length - 2);
                const moreBenchText = moreBenchCount > 0 ? ` 외 ${moreBenchCount}마리` : '';
                log += ` 대기 펫 ${shownBenchNames}${moreBenchText}도 꽃물살의 여운으로 합계 ${benchHealTotal} 회복했습니다!`;
            }
    ...
    ARA_BLOOM: {
        ...
        element: '물',
        basePower: 36,
        description: '바다 깊은 곳의 꽃물살을 만개시켜 표식을 회수합니다. 상대의 물결표식이 1/2/3개일 때 피해가 각각 1.4배/2.5배/4.5배로 증가하며, 명중하면 표식은 모두 사라집니다.',
...
,
    ...
    FROST_CLAW: {
        ...
        basePower: 22,
        description: '차가운 서리가 맺힌 손톱으로 상대를 할퀴어 얼음 피해를 주고, 상대 현재 체력의 6%만큼 자신의 체력을 회복합니다. 초과 회복은 되지 않습니다.',
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            const attacker = attackerPlayer.pet;
            const defender = defenderPlayer.pet;
            ...
            const siphonBase = Math.max(0, Math.floor(defenderHpBefore * 0.06));
