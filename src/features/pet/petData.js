...
export const SKILLS = {
    ...
    HEALING_PRAYER: {
        id: 'healing_prayer', name: '회복의 기도', cost: 35, type: 'common', element: null,
    ...
    ENERGY_SIPHON: {
        id: 'energy_siphon', name: '에너지 사이펀', cost: 20, type: 'common', element: null,
    ...
    FIERY_BREATH: {
        ...
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            ...
            let { damage, isEffective, isCritical: breathCrit } = calculateDamage(SKILLS.FIERY_BREATH.basePower, attackerPlayer, defenderPlayer, SKILLS.FIERY_BREATH.element, 5.5, 1.2);
    ...
    DRAGON_CLAW: {
        ...
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            const attacker = attackerPlayer.pet; const defender = defenderPlayer.pet;
            let { damage, isEffective, isCritical: clawCrit } = calculateDamage(35, attackerPlayer, defenderPlayer, '불', 2.7, 1.5);
    ...
    STELLAR_BLAST: {
        id: 'stellar_blast', name: '스텔라 블라스트', cost: 65, type: 'signature', element: '불', basePower: 60,
        description: '초고열의 항성 에너지를 폭발시킵니다. 30% 확률로 상대를 매 턴 최대 체력의 8%씩 화상 도트 피해 상태로 만듭니다.',
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            const attacker = attackerPlayer.pet; const defender = defenderPlayer.pet;
            let { damage, isEffective, isCritical: stelCrit } = calculateDamage(60, attackerPlayer, defenderPlayer, '불', 7.0, 1.2);
            if (defenderAction === 'BRACE') damage *= 0.7;
            defender.hp = Math.max(0, defender.hp - Math.round(damage));
    ...
    QUICK_DISTURBANCE: {
        id: 'quick_disturbance', name: '재빠른 교란', cost: 18, type: 'signature', element: '바람', basePower: 20,
    ...
    WIND_BLADE: {
        id: 'wind_blade', name: '바람의 칼날', cost: 28, type: 'signature', element: '바람', basePower: 30,
    ...
    TORNADO_SWEEP: {
        id: 'tornado_sweep', name: '토네이도 휩쓸기', cost: 70, type: 'signature', element: '바람', basePower: 60,
    ...
    VINE_WHIP: {
        id: 'vine_whip', name: '덩굴 채찍', cost: 28, type: 'signature', element: '풀', basePower: 35,
    ...
    SOLAR_BEAM: {
        id: 'solar_beam', name: '솔라 빔', cost: 65, type: 'signature', element: '풀', basePower: 65,
    ...
    THUNDER_PUNCH: {
        id: 'thunder_punch', name: '찌릿펀치', cost: 30, type: 'signature', element: '번개', basePower: 40,
    ...
    THUNDERSTORM: {
        id: 'thunderstorm', name: '뇌우', cost: 70, type: 'signature', element: '번개', basePower: 65,
    ...
    FLAME_DASH: {
        ...
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            ...
            let { damage, isCritical: dashCrit } = calculateDamage(SKILLS.FLAME_DASH.basePower, attackerPlayer, defenderPlayer, '불', 2.8, 0.6);
            damage = Math.round(damage);
            defender.hp = Math.max(0, defender.hp - damage);
    ...
    UPHWA: {
        id: 'uphwa', name: '업화', cost: 70, type: 'signature', element: '불', basePower: 65,
    ...
    ULTIMATE_SECRET: {
        id: 'ultimate_secret', name: '오의필살', cost: 90, type: 'signature', element: null, basePower: 70,
    ...
    REED_BOW: {
        ...
        description: '부들 화살을 쏴 약간의 피해를 주고, 상대를 2턴간 속박하여 방어 행동과 도망치기를 봉쇄합니다.',
        effect: (attackerPlayer, defenderPlayer, defenderAction) => {
            const attacker = attackerPlayer.pet; const defender = defenderPlayer.pet;
            if (!defender.status) defender.status = {};
...
export const PET_DATA = {
    ['dragon']: {
        ...
        evolution: {
            lv10: { appearanceId: 'dragon_lv2', name: '은하룡', statBoost: { hp: 1.5, sp: 1.3, atk: 1.5 }, newSkill: SKILLS.DRAGON_CLAW, description: "별빛의 은하 마력을 다듬어 한층 더 강력한 화염을 내뿜는 은하룡입니다. (🔥불 속성)" },
            lv20: { appearanceId: 'dragon_lv3', name: '스텔라곤', statBoost: { hp: 2.2, sp: 1.6, atk: 2.2 }, newSkill: SKILLS.STELLAR_BLAST, description: "우주의 모든 항성 에너지를 지배하는 위엄 넘치는 고대 용의 황제 스텔라곤입니다. (🔥불 속성)" },
        }
    },
    ['rabbit']: {
        name: '버니니',
        ...
        evolution: {
            lv10: { appearanceId: 'rabbit_lv2', name: '버닉스', statBoost: { hp: 1.4, sp: 1.6, atk: 1.4 }, newSkill: SKILLS.WIND_BLADE, description: "압축된 바람을 날카로운 칼날처럼 자유자재로 다루는 날렵한 투사 버닉스입니다. (💨바람 속성)" },
            lv20: { appearanceId: 'rabbit_lv3', name: '하이버닉스', statBoost: { hp: 1.9, sp: 2.2, atk: 1.8 }, newSkill: SKILLS.TORNADO_SWEEP, description: "거대한 토네이도를 일으켜 전장을 휩쓰는 폭풍의 지배자 하이버닉스입니다. (💨바람 속성)" },
        }
    },
    ['turtle']: {
        name: '새싹치',
        ...
        evolution: {
            lv10: { appearanceId: 'bird_lv2', name: '꽃잎치', statBoost: { hp: 1.6, sp: 1.2, atk: 1.35 }, newSkill: SKILLS.VINE_WHIP, description: "향기로운 꽃이 만개하여 넘치는 생명력으로 무장한 꽃잎치입니다. (🌿풀 속성)" },
            lv20: { appearanceId: 'bird_lv3', name: '열매치', statBoost: { hp: 2.3, sp: 1.5, atk: 2.0 }, newSkill: SKILLS.SOLAR_BEAM, description: "태양의 에너지를 가득 머금은 생명의 결실을 품고 파괴적인 빛을 쏘는 열매치입니다. (🌿풀 속성)" },
        }
    },
    ['monkey']: {
        name: '찌릿숭이',
        ...
        evolution: {
            lv10: { appearanceId: 'monkey_lv2', name: '지직숭', statBoost: { hp: 1.5, sp: 1.5, atk: 1.5 }, newSkill: SKILLS.THUNDER_PUNCH, description: "고압 전류를 양 주먹에 감고 강력한 전격 타격을 연마한 격투가 지직숭입니다. (⚡번개 속성)" },
            lv20: { appearanceId: 'monkey_lv3', name: '콰릉숭', statBoost: { hp: 2.0, sp: 2.1, atk: 1.9 }, newSkill: SKILLS.THUNDERSTORM, description: "하늘에서 거대한 천둥과 뇌우를 떨어뜨려 전장을 초토화시키는 콰릉숭입니다. (⚡번개 속성)" },
        }
    },
    ['fox']: {
        name: '모롱이',
        ...
        evolution: {
            lv10: { appearanceId: 'fox_lv2', name: '불비비', statBoost: { hp: 1.5, sp: 1.4, atk: 1.5 }, newSkill: SKILLS.FLAME_DASH, description: "꼬리의 불꽃이 거세게 타오르며 적의 방어를 꿰뚫고 맹렬히 질주하는 불비비입니다. (🔥불 속성)" },
            lv20: { appearanceId: 'fox_lv3', name: '인페르노', statBoost: { hp: 2.2, sp: 1.8, atk: 2.1 }, newSkill: SKILLS.UPHWA, description: "귀엽나요? 역전의 지옥 화염을 다루며 모든 것을 태워버리는 불멸의 파괴신 인페르노입니다. (🔥불 속성)" },
        }
    },
    ['frog']: {
        name: '미소구리',
