...
async function tryMatchRandomTeamQueue(classId, playerId) {
  ...
  // TEAM_REPEAT_AVOIDANCE_HISTORY_PATCH
  // chooseBalancedTeamSplit 자체는 같은 팀/상대 반복 페널티 로직을 이미 갖고 있었지만,
  // 실제 "누구랑 오늘 같은 팀이었는지" 데이터가 한 번도 전달되지 않아서 항상 0으로 계산되고
  // 있었습니다. 여기서 실제 이력을 읽어와 채워 넣습니다.
  const today = getTodayString();
  const historySnaps = await Promise.all(
    pool.map((entry) => getDoc(playerRefOf(classId, entry.playerId)))
  );
  const historyByPlayerId = Object.fromEntries(
    historySnaps.map((snap, index) => [pool[index].playerId, snap.exists() ? snap.data() : {}])
  );

  const sameTeamPairCounts = {};
  const opponentPairCounts = {};
  const pairKeyLocal = (a, b) => [a, b].filter(Boolean).sort().join('__');

  for (let i = 0; i < pool.length; i += 1) {
    for (let j = i + 1; j < pool.length; j += 1) {
      const a = pool[i].playerId;
      const b = pool[j].playerId;
      const aData = historyByPlayerId[a] || {};
      const bData = historyByPlayerId[b] || {};

      const aTeammatesToday = aData.randomTeamTodayTeammatesDate === today
        ? (aData.randomTeamTodayTeammates || [])
        : [];
      const aOpponentsToday = aData.randomTeamTodayOpponentsDate === today
        ? (aData.randomTeamTodayOpponents || [])
        : [];

      if (aTeammatesToday.includes(b)) {
        sameTeamPairCounts[pairKeyLocal(a, b)] = (sameTeamPairCounts[pairKeyLocal(a, b)] || 0) + 1;
      }
      if (aOpponentsToday.includes(b)) {
        opponentPairCounts[pairKeyLocal(a, b)] = (opponentPairCounts[pairKeyLocal(a, b)] || 0) + 1;
      }
      // 양쪽 중 한 쪽에만 기록이 있어도 반영되도록 상대방 기록도 같이 확인합니다.
      const bTeammatesToday = bData.randomTeamTodayTeammatesDate === today
        ? (bData.randomTeamTodayTeammates || [])
        : [];
      if (bTeammatesToday.includes(a) && !aTeammatesToday.includes(b)) {
        sameTeamPairCounts[pairKeyLocal(a, b)] = (sameTeamPairCounts[pairKeyLocal(a, b)] || 0) + 1;
      }
    }
  }
  ...
  // TEAM_REPEAT_AVOIDANCE_HISTORY_PATCH
  // 매칭이 실제로 성사됐을 때만, 다음 매칭에 쓰일 "오늘 같은 팀/상대" 이력을 남깁니다.
  if (matchResult?.matched) {
    await recordTeamMatchHistory(classId, matchResult.teamAIds, matchResult.teamBIds, today);
  }
...
/**
 * 팀대전 매칭이 끝난 뒤 "오늘 누구랑 같은 팀/상대였는지"를 각자 플레이어 문서에 남깁니다.
 * chooseBalancedTeamSplit이 다음 매칭 때 이 데이터를 읽어 같은 조합 반복을 피하는 데 씁니다.
 */
async function recordTeamMatchHistory(classId, teamAIds, teamBIds, today) {
  const allIds = [...teamAIds, ...teamBIds];

  await Promise.all(allIds.map(async (playerId) => {
    const isTeamA = teamAIds.includes(playerId);
    const myTeammates = (isTeamA ? teamAIds : teamBIds).filter((id) => id !== playerId);
    const myOpponents = isTeamA ? teamBIds : teamAIds;

    const playerRef = playerRefOf(classId, playerId);
    const snap = await getDoc(playerRef);
    const data = snap.exists() ? snap.data() : {};

    const prevTeammates = data.randomTeamTodayTeammatesDate === today
      ? (data.randomTeamTodayTeammates || [])
      : [];
    const prevOpponents = data.randomTeamTodayOpponentsDate === today
      ? (data.randomTeamTodayOpponents || [])
      : [];

    await setDoc(playerRef, {
      randomTeamTodayTeammates: [...new Set([...prevTeammates, ...myTeammates])],
      randomTeamTodayTeammatesDate: today,
      randomTeamTodayOpponents: [...new Set([...prevOpponents, ...myOpponents])],
      randomTeamTodayOpponentsDate: today,
    }, { merge: true });
  }));
}
