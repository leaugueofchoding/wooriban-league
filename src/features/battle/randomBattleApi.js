...
export async function setRandomBattlePresetPetIds(classId, playerId, battlePresetPetIds) {
  ...
  return await runTransaction(db, async (transaction) => {
    ...
    Object.values(preset).filter(Boolean).forEach((petId) => {
      if (!ownedPetIds.has(petId)) throw new Error('보유하지 않은 펫은 랜덤 대전 프리셋에 넣을 수 없습니다.');
    });
...
export async function useVitaminJellyForPet(classId, playerId, petId) {
  ...
  return await runTransaction(db, async (transaction) => {
    ...
    if (isActiveQueueStatus(queueData?.status)) throw new Error('큐 신청 후에는 비타민젤리를 사용할 수 없습니다.');
...
export async function createRandomBattleQueueEntry(classId, playerId, options = {}) {
  ...
  const queueRef = queueRefOf(classId, playerId);
  const today = getTodayString();
  const teamSize = Number(options.teamSize || RANDOM_BATTLE_CONFIG.TEAM_SIZE_1V1);
  ...
  return await runTransaction(db, async (transaction) => {
    ...
    const resolvedTeam = resolveRandomBattleTeam({
      player: playerData,
      today,
      lockedPetIds: Array.isArray(options.lockedPetIds) ? options.lockedPetIds : [],
      teamSize,
    });
...
export async function consumeRandomBattleFatigueForPets(classId, playerId, petIds, options = {}) {
  assertClassAndPlayer(classId, playerId);
  if (!Array.isArray(petIds) || petIds.length === 0) throw new Error('대전 횟수를 차감할 펫 정보가 없습니다.');
  ...
  return await runTransaction(db, async (transaction) => {
    ...
    const playerData = playerSnap.data();
    const nextPets = applyBattleFatigueToPets({ pets: playerData.pets || [], petIds, today });
...
export async function confirmRandomBattleEntrance(classId, playerId) {
  ...
  return await runTransaction(db, async (transaction) => {
    ...
    transaction.set(queueRef, { status: 'entering', entrantConfirmedAt: serverTimestamp() }, { merge: true });
