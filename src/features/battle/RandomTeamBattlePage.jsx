...
import { startTeamBattleFight } from './teamBattleStart';
...
const Header = styled.div`
  ...
  h2 { margin: 0; color: #343a40; font-size: 1.35rem; font-weight: 1000; }
  p { margin: 0.25rem 0 0; color: #868e96; font-size: 0.86rem; font-weight: 800; }
`;
...
const Member = styled.div`
  ...
  strong { display: block; color: #343a40; font-size: 0.94rem; font-weight: 1000; }
  span { display: block; color: #868e96; font-size: 0.78rem; font-weight: 850; }
`;
...
const Button = styled.button`
  ...
  font-weight: 1000;
  cursor: pointer;
  background: ${props => props.$muted ? '#868e96' : '#5f3dc4'};
  box-shadow: 0 4px 0 rgba(0,0,0,0.14);

  &:disabled { background: #adb5bd; box-shadow: none; cursor: not-allowed; }
`;

const imageFor = (appearanceId) => {
...
function RandomTeamBattlePage() {
  ...
  useEffect(() => {
    ...
    if (room.fightBattleId) {
      navigate(`/battle/team-fight/${encodeURIComponent(room.fightBattleId)}`, { replace: true });
      return;
    }

    startAttemptedRef.current = true;
    startTeamBattleFight(classId, matchId)
      .then((result) => {
        if (result?.battleId) {
          navigate(`/battle/team-fight/${encodeURIComponent(result.battleId)}`, { replace: true });
        }
      })
      .catch((error) => {
        console.error('팀대전 시작 실패:', error);
        startAttemptedRef.current = false;
      });
  }, [classId, matchId, room?.status, room?.fightBattleId, navigate]);
    ...
    return (
      <Member key={'blind-' + member.playerId}>
        <div style={{
          ...
          boxShadow: isMemberReady ? '0 0 0 4px #ebfbee' : 'none', flex: '0 0 auto',
        }} />
        <div>
  ...
  const renderMember = (member) => {
    ...
    const imageSrc = imageFor(pet.appearanceId);
    return (
      ...
      <Card>
        ...
        {!room ? (
          ...
          <>
            ...
            <ButtonRow>
              <Button type="button" onClick={handleReady} disabled={isProcessing || isReady || allReady}>
                {isReady ? '입장 완료' : isProcessing ? '처리 중...' : '입장 확인'}
              </Button>
              <Button type="button" $muted onClick={() => navigate('/pet')}>펫 페이지</Button>
            </ButtonRow>
          </>
        )}
      </Card>
