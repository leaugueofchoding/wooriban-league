...
// RANDOM_BATTLE_QUEUE_M4_PAGE_SCAFFOLD
...
const PetCard = styled.div`
  ...
  border-radius: 18px;
  padding: 0.85rem;
  background: ${props => props.$available ? '#ffffff' : '#f1f3f5'};
  display: grid;
  gap: 0.45rem;
  text-align: center;
  min-height: 210px;
...
function RandomBattlePage() {
  ...
  const handleEnterBattle = async () => {
    ...
    setError('');
    try {
      const updatedQueue = await confirmRandomBattleEntrance(classId, myPlayerData.id);
      if (updatedQueue?.matchedBattleId) {
        navigate(`/battle/${updatedQueue.matchedBattleId}`);
      } else {
  ...
  if (!myPlayerData) {
    return (
      <Page>
        <Shell>
          <ErrorBox>플레이어 정보를 불러오는 중입니다.</ErrorBox>
        </Shell>
      </Page>
    );
  }
  ...
  return (
    <Page>
      <Shell>
        <HeroCard>
          ...
          <StatusCard>
            ...
            {queueEntry?.status === 'waiting' && (
              <HelpText>상대를 찾는 중입니다... 비슷한 레벨의 상대를 찾고 있어요.</HelpText>
            )}
            {isMatched && (
              <HelpText>🎲 랜덤 대전이 매칭되었습니다! 상대는 아직 공개되지 않습니다. 입장하면 상대와 펫이 공개됩니다.</HelpText>
            )}
          </StatusCard>

          <ActionRow>
            {!activeQueue && (
              <Button onClick={handleJoinQueue} disabled={isSubmitting || !resolvedTeam.isComplete}>
                랜덤 대전 신청
              </Button>
            )}
            {queueEntry?.status === 'waiting' && (
              <Button $variant="secondary" onClick={() => handleCancelQueue('cancelled_by_player')} disabled={isSubmitting}>
                대기 취소
              </Button>
            )}
            {isMatched && (
              <>
                <Button onClick={handleEnterBattle} disabled={isSubmitting}>
                  입장하기
                </Button>
                <Button $variant="secondary" onClick={() => handleCancelQueue('skip_matched_battle')} disabled={isSubmitting}>
                  이번엔 쉬기
                </Button>
              </>
            )}
        ...
        <NoticeBox>
          이 화면은 M4 랜덤 1:1 큐의 학생 UX 뼈대입니다. 다음 단계에서 매칭 워커/방 생성 로직이 붙으면
          20초 입장 제한, 양쪽 입장 완료 후 상대 공개, 배틀 시작 시 피로도 차감까지 연결됩니다.
        </NoticeBox>
      </Shell>
    </Page>
  );
