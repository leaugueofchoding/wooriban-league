...
export const useLeagueStore = create((set, get) => ({
    ...
    fetchInitialData: async () => {
        try {
            ...
            if (!activeSeason) {
                ...
                return set({ isLoading: false, players: [], teams: [], matches: [] });
            }
    ...
    startSeason: async () => {
        ...
        try {
            ...
            await get().fetchInitialData(); // 상태 변경 후 데이터 즉시 다시 로드
        } catch (error) {
    ...
    createNewSeason: async () => {
        const season = get().currentSeason;
        if (!season || season.status !== 'completed') return alert('현재 시즌이 종료되어야 새 시즌을 만들 수 있습니다.');
        if (!confirm('지난 시즌이 종료되었습니다. 다음 시즌을 준비하시겠습니까?')) return;

        // "25 - 1시즌" -> "25 - 2시즌" 으로 이름 자동 생성
        const nameParts = season.seasonName.split(' - ');
        const year = nameParts[0];
        const num = parseInt(nameParts[1]) + 1;
        const newSeasonName = `${year} - ${num}시즌`;
        ...
        try {
            ...
            alert(`${newSeasonName}이 준비 상태로 생성되었습니다.`);
            get().fetchInitialData(); // 최신 시즌 정보를 다시 불러옵니다.
        } catch (error) {
            console.error("새 시즌 생성 오류:", error);
            alert('새 시즌을 만드는 중 오류가 발생했습니다.');
        }
    },
    ...
    autoAssignTeams: async () => {
        ...
        if (players.length === 0 || teams.length === 0) return alert('선수와 팀이 모두 있어야 자동 배정을 할 수 있습니다.');
    ...
    generateSchedule: async () => {
        if (!confirm('기존 경기 일정이 모두 삭제되고 새로 생성됩니다. 계속하시겠습니까?')) return;
        const { teams, leagueType, currentSeason } = get();
        if (!currentSeason) return alert('현재 시즌 정보가 없습니다.');
        if (teams.length < 2) return alert('경기 일정을 생성하려면 최소 2팀이 필요합니다.');
        ...
        if (matchesToCreate.length === 0) return alert('생성할 경기 일정이 없습니다.');
