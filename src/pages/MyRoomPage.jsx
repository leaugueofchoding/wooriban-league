...
function MyRoomPage() {
  ...
  const { players, myRoomItems, avatarParts, titles, fetchInitialData } = useLeagueStore();
...
movementActor: {
  ...
  const myRoomDisplayLikes = useMemo(() => {
    // MYROOM_CUMULATIVE_LIKES_DISPLAY_V4
    return Math.max(
      Number(roomOwnerData?.myRoomLikesTotal || 0),
      Number(roomOwnerData?.myRoomLikeCount || 0),
      Number(roomOwnerData?.myRoomLikesCount || 0),
      likes.length
    );
...
const itemCounts = useMemo(() => (roomConfig.items || []).reduce((acc, item) => { acc[item.itemId] = (acc[item.itemId] || 0) + 1; return acc; }, {}), [roomConfig.items]);
  ...
  const handlePostComment = async () => {
    ...
    // MYROOM_LIKE_REFRESH_V4
    if (!myPlayerData) return alert('로그인 후 이용해주세요.');
    if (hasLikedThisMonth) return;

    try {
      const result = await likeMyRoom(classId, playerId, myPlayerData.id, myPlayerData.name);

      setLikes(prev => {
        if (prev.some(like => like.id === myPlayerData.id)) return prev;
        return [
          ...prev,
          {
            id: myPlayerData.id,
            likerName: myPlayerData.name,
            lastLikedMonth: new Date().toISOString().slice(0, 7)
          }
        ];
      });

      if (result?.myRoomLikesTotal && roomOwnerData?.id === playerId) {
        roomOwnerData.myRoomLikesTotal = result.myRoomLikesTotal;
      }
        ...
        <HeaderActions>
          {!isMyRoom && (
            <>
              <LikeButton onClick={handleLikeRoom} disabled={hasLikedThisMonth} $active={hasLikedThisMonth}>
                {hasLikedThisMonth ? '❤️' : '🤍'} 좋아요 {myRoomDisplayLikes}
              </LikeButton>
          ...
          {isMyRoom && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#fa5252' }}>❤️ {myRoomDisplayLikes}</span>
              {/* ▼▼▼ [추가] 방 주인: 누가 좋아요 눌렀는지 확인 */}
              {likes.length > 0 && (
                <button
                  onClick={() => {
                    const names = likes.map(l => l.likerName || l.name || l.id).join('\n');
                    alert(`❤️ 이번 달 좋아요를 눌러준 친구들 (${likes.length}명):\n\n${names}`);
                  }}
